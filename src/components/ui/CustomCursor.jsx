...
const CustomCursor = () => {
   ...
   useEffect(() => {
      // Basic mobile detection (width + touch capability)
      const isMobile =
         window.innerWidth <= 768 || window.matchMedia("(pointer: coarse)").matches

      if (isMobile) {
         // Restore cursor just in case CSS didn't catch it
      ...
      const addListeners = () => {
         const interactables = document.querySelectorAll(
            "a, button, input, textarea, .hamburger, .manual-btn, .wraper > div, .team_member",
         )
      ...
      <>
         ...
         <div
            ...
            style={{
               ...
               zIndex: 9998,
               transition:
                  "transform 0.1s ease, background-color 0.2s ease, border-color 0.2s ease",
            }}
         />
      </>
   )
